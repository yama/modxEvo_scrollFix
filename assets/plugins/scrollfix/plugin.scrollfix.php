 ...
 *              &scrollFix=ScrollFix;list;Enabled,Disabled;Disabled &fixTabHeader=Fix Tab-Header;list;Enabled,Disabled;Disabled &cookieLifetime=ScrollFix Cookie-Lifetime (Minutes);int;120 &addToTopButton=Add ToTop-Button;list;Enabled,Disabled;Disabled &addToTopCustom=ToTop-Button Custom-CSS (replace semicolon by |);textarea;bottom:15px|right:40px|width:40px|height:40px|line-height:40px|font-size:20px|background-color:#89AD4A|color:#fff|border:1px solid #658F1A|border-radius:5px &addToTopLabel=ToTop-Button Custom-Label;text &extSaveButtons=Extend Save-Button;list;Enabled,Disabled;Disabled &alwaysStay=AlwaysStay;list;Enabled,Disabled;Disabled &jQueryCdn=Load jQuery from CDN (code.jquery.com);list;Enabled,Disabled;Disabled
 * @internal    @events:
...
$addToTopCustom = isset( $addToTopCustom ) ? $addToTopCustom : 'bottom:15px|right:40px|width:40px|height:40px|line-height:40px|font-size:20px|background-color:#89AD4A|color:#fff|border:1px solid #658F1A|border-radius:5px';
...
if ( $e->name == "OnManagerMainFrameHeaderHTMLBlock" ) {
	...
	$toTopFunc = ( $modx11 && $fixTabHeader == 'Disabled') ? 'window.scrollTo(0,70)' : 'setScrollXY(0,0)';
	$addToTopCustom = str_replace('|',';',$addToTopCustom);
	$jWinLoad[] = '$j("body").append("<a onclick=\"'. $toTopFunc .'\" style=\"position:fixed;z-index:999999;display:block;text-align:center;cursor:pointer;text-decoration:none;'. $addToTopCustom .'\">'. $addToTopLabel .'</a>");';
    };

    // PREPARE EXTENDED SAVE-BUTTON
    if( $extSaveButtons != 'Disabled' ) {
	global $_style, $_lang;
	$jDocReady[] = '
			$j("#stay").hide();
			$j("#stay").val(2);
			$j("#Button1").attr("title","'. $_lang["save"] .' + '. $_lang["stay"] .'");
			$j("#Button1").append("<a style=\"cursor: pointer;padding-left:1em;padding-right:1em;\" onclick=\"$j(\'#stay\').val(\'1\'); documentDirty=false; document.mutate.save.click();saveWait(\'mutate\');\" title=\"'. $_lang["save"] .' + '. $_lang["stay_new"] .'\"><img src=\"'. $_style["icons_new_document"] .'\" /></a>");
			$j("#Button1").append("<a style=\"cursor: pointer;padding-left:1em;padding-right:1em;margin-right:1em;\" onclick=\"$j(\'#stay\').val(\'\'); documentDirty=false; document.mutate.save.click();saveWait(\'mutate\');\" title=\"'. $_lang["save"] .' + '. $_lang["close"] .'\"><img src=\"'. $_style["icons_cancel"] .'\" /></a>");
';
    };
